...
**Relationship and retention data isn't independently queryable yet.** To keep deployments migration-free, the cross-table links are folded into each finding's `detection_reason` text and the per-table data ages live inside the scan's `classifier_stats` JSON blob — not in dedicated, indexed columns. That's perfectly fine for rendering everything on the scan page, but it means you can't yet run a query like "show me every table across all sources holding data older than seven years" without unpacking JSON by hand. The retention number for a table also only lives on the scan that produced it, so historical scans from before this feature won't show one. Promoting these into proper columns (and a relationships table) is a clean migration whenever it's needed — the current design just trades queryability for the convenience of never having to touch the production database.
